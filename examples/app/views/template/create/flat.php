                ...
                <div class="form-group">
                    <table class="table table-bordered" id="dynamic_head_actions">
                        <tr>
                            ...
                            <td>
                                Название шаблона
                                <input type="text" id="templateName" name="templateName" placeholder="Введите название шаблона" class="form-control name_list"/>
                            </td>
                            <td>
                                Название таблицы
                                <input type="text" id="tableName" name="tableName" placeholder="Введите название таблицы" class="form-control name_list"/>
                            </td>
                        </tr>
                        <tr>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered" id="dynamic">
                                ...
                                <tr>
                                    ...
                                    <td><select name="status[]" class="form-control name_list">
                                            <option value="-1">Выберите </option>
                                            <option value="1">Акктивный</option>
                                            <option value="2">Только для шапки</option>
                                        </select></td>
                                    </td>
                    ...
                    <input type="button" id="createTemplate" class="btn btn-info" value="Добавить">
                </div>
            </div>
        </div>
    </div>
    ...
    $(document).ready(function() {
        ...
        $('#add').click(function(){ // при нажатии на кнопку ДОБАВИТЬ ПОЛЕ
            ...
            $('#dynamic').append( // добавляем еще поля в таблице
                ...
                '<td><input type="text" name="sort_order[]" placeholder="Сортировка на уровне" class="form-control name_list" /></td> ' +
                '<td>' +
                '<select name="status[]" class="form-control name_list"> ' +
                '<option value="-1">Выберите </option> ' +
                '<option value="1">Создавать поле</option> ' +
                '<option value="2">Поле только для шапки</option> ' +
                '</select>' +
                '</td>' +
                '<td><input type="text" name="field_name[]" placeholder="Название поля в таблице" class="form-control name_list" /></td> ' +
                '<td><input type="text" name="name[]" placeholder="Название для шапки" class="form-control name_list" /></td> ' +
        ...
        $('#createTemplate').click(function(){
            if($('#tblName').val() != '' && $('#templateName').val() != ''){
                $.ajax({
                    type: 'post',
                    url: '/template/create/flat',
                    data: {
                        form: $('#dynamic-insert').serializeObject(),
                        tableName: $('#tableName').val(),
                        templateName: $('#templateName').val()
                    },
                    beforeSend: function () {
                        $("#createTemplate").text('Loading...Please, wait...');
                    },
